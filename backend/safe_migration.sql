-- ==========================================
-- SAFE ADDITIVE SUPABASE MIGRATION SCRIPT
-- ==========================================
-- Only creates missing tables. Does NOT drop data.
-- Preserves existing users and profiles by using IF NOT EXISTS.
-- Specifically omits 'doctors' and 'appointments' features.
...
-- Add missing column safely if it didn't exist
...
DROP POLICY IF EXISTS "Users can view own predictions" ON disease_predictions;
...
DROP POLICY IF EXISTS "Users can insert own predictions" ON disease_predictions;
...
-- PostgREST Schema Cache reload instructions:
-- Running this schema modification automatically invalidates the schema cache for most Supabase setups.
-- If the application still reports PGRST205, you can run: NOTIFY pgrst, 'reload schema';
