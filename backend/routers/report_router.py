...
async def get_user_reports(user_id: str, user: Any = Depends(verify_token)):
    ...
    try:
        ...
        # Transform the response to extract report_data directly so it matches the frontend's Report interface
        reports = []
        for r in res.data:
            report = r.get("report_data", {})
            report["id"] = r["id"] # Overwrite ID with DB ID just in case
